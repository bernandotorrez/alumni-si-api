...
class CreateAlumni {
  get rules () {
    return {
      ...
      nama: 'required|alpha_numeric|min:3|max:100',
      jenis_kelamin: 'required|in:L,P',
      tempat_lahir: 'required|alpha_numeric|min:3|max:25',
      tanggal_lahir: 'required|date|date_format:YYYY-MM-DD',
      no_hp: 'required|integer|min:10|max:15',
      alamat: 'required|alpha_numeric|min:6',
      angkatan_masuk: 'required|integer|min:4|max:4',
      fakultas: 'required|alpha_numeric|min:10|max:100',
      jurusan: 'required|alpha_numeric|min:5|max:50'
    }
  }
